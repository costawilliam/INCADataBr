...
numeroCasosPorsexoeUF <- function(...) {
  ...
  if(is.na(params$pathFile)){
    ...
    #df <- lerArquivoDBF(params$pathFile)
    df <- lerArquivoDBF("C:/Users/WilliamCostaJacksond/Documents/William/coisas tc/TC II/Dados/2017/rhc17.dbf")
    colnames(df) <-  tolower(colnames(df))
    library(plyr)
    df <- count(df, vars = c("sexo", "ufuh"))

    df <- data.frame(sexo = converterFatorParaInteiro(df$sexo), ufuh = converterFatorParaCaracter(df$ufuh), nrocasos = df$freq )
  }
